...
Only corroborated auto_strong/auto_weak/manual matches produce a public link. Name-only
probabilistic matches wait in the review queue and are NOT shown publicly.
...
def resolve(session: Session, *, year: int | None = None) -> ResolutionResult:
    ...
    for cand in cands:
        ...
        if not hit.corroborated or hit.is_homonym or hit.score < WEAK:
            if not hit.corroborated:
                reason = "name only (no CPF/birth date)"
            else:
        ...
        # At this point the match has corroborating identity data, so a strong score
        # may be published without relying on name similarity alone.
        tier = (
            ConfidenceTier.auto_strong
    ...
    # Remove stale links generated by an earlier run before upserting the current
    # result. Manual decisions are authoritative and use a different resolver.
    session.execute(
        CandidateMandateLink.__table__.delete().where(
            CandidateMandateLink.sq_candidato.in_([c.sq_candidato for c in cands]),
            CandidateMandateLink.resolver == PIPELINE_VERSION,
        )
    )

    # Persist. Reviews never clobber decided rows.
    upsert(
        session,
        CandidateMandateLink,
        link_rows,
